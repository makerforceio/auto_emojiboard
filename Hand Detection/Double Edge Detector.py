...
while(True):
    ...
    for i in range(min(3, len(contours))):
        ...
        edges = cv.Canny(hand,cv.getTrackbarPos('bot','frame'),cv.getTrackbarPos('top','frame'))
        edges = edges[3:-3, 3:-3]
        edge_contours = cv.findContours(edges, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_SIMPLE)[1]
        edge_contours = sorted(edge_contours, key=cv.contourArea, reverse=True)
        cv.drawContours(hand, edge_contours, -1, (0, 255, 0), 3)
        cv.imshow('edges{}'.format(i), edges)
