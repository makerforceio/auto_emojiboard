...
import io.makerforce.inputmethod.keyboard.internal.KeySpecParser;
import io.makerforce.inputmethod.keyboard.internal.KeyStyle;
...
import io.makerforce.inputmethod.keyboard.internal.KeyboardRow;
...
public class Key implements Comparable<Key> {
    ...
    public Key(@Nullable final String keySpec, @Nonnull final TypedArray keyAttr,
        ...
        mBackgroundType = style.getInt(keyAttr,
                io.makerforce.inputmethod.latin.R.styleable.Keyboard_Key_backgroundType, row.getDefaultBackgroundType());

        final int baseWidth = params.mBaseWidth;
        final int visualInsetsLeft = Math.round(keyAttr.getFraction(
                io.makerforce.inputmethod.latin.R.styleable.Keyboard_Key_visualInsetsLeft, baseWidth, baseWidth, 0));
        final int visualInsetsRight = Math.round(keyAttr.getFraction(
                io.makerforce.inputmethod.latin.R.styleable.Keyboard_Key_visualInsetsRight, baseWidth, baseWidth, 0));

        mLabelFlags = style.getFlags(keyAttr, io.makerforce.inputmethod.latin.R.styleable.Keyboard_Key_keyLabelFlags)
                | row.getDefaultKeyLabelFlags();
        final boolean needsToUpcase = needsToUpcase(mLabelFlags, params.mId.mElementId);
        final Locale localeForUpcasing = params.mId.getLocale();
        int actionFlags = style.getFlags(keyAttr, io.makerforce.inputmethod.latin.R.styleable.Keyboard_Key_keyActionFlags);
        String[] moreKeys = style.getStringArray(keyAttr, io.makerforce.inputmethod.latin.R.styleable.Keyboard_Key_moreKeys);

        // Get maximum column order number and set a relevant mode value.
        int moreKeysColumnAndFlags = MORE_KEYS_MODE_MAX_COLUMN_WITH_AUTO_ORDER
                | style.getInt(keyAttr, io.makerforce.inputmethod.latin.R.styleable.Keyboard_Key_maxMoreKeysColumn,
        ...
        if ((mLabelFlags & LABEL_FLAGS_DISABLE_ADDITIONAL_MORE_KEYS) != 0) {
            ...
            additionalMoreKeys = style.getStringArray(keyAttr,
                    io.makerforce.inputmethod.latin.R.styleable.Keyboard_Key_additionalMoreKeys);
        }
        ...
        final int disabledIconId = KeySpecParser.getIconId(style.getString(keyAttr,
                io.makerforce.inputmethod.latin.R.styleable.Keyboard_Key_keyIconDisabled));
        ...
        if ((mLabelFlags & LABEL_FLAGS_DISABLE_HINT_LABEL) != 0) {
            ...
            final String hintLabel = style.getString(
                    keyAttr, io.makerforce.inputmethod.latin.R.styleable.Keyboard_Key_keyHintLabel);
        ...
        final int altCodeInAttr = KeySpecParser.parseCode(
                style.getString(keyAttr, io.makerforce.inputmethod.latin.R.styleable.Keyboard_Key_altCode), Constants.CODE_UNSPECIFIED);
