...
package io.makerforce.inputmethod.latin.settings;
...
import io.makerforce.inputmethod.compat.AppWorkaroundsUtils;
import io.makerforce.inputmethod.latin.InputAttributes;
import io.makerforce.inputmethod.latin.RichInputMethodManager;
import io.makerforce.inputmethod.latin.utils.AsyncResultHolder;
import io.makerforce.inputmethod.latin.utils.ResourceUtils;
import io.makerforce.inputmethod.latin.utils.TargetPackageInfoGetterTask;
import io.makerforce.inputmethod.latin.utils.RunInLocale;
...
public class SettingsValues {
    ...
    public SettingsValues(final Context context, final SharedPreferences prefs, final Resources res,
        ...
        mDelayInMillisecondsToUpdateOldSuggestions =
                res.getInteger(io.makerforce.inputmethod.latin.R.integer.config_delay_in_milliseconds_to_update_old_suggestions);
        ...
        final String autoCorrectionThresholdRawValue = mAutoCorrectEnabled
                ? res.getString(io.makerforce.inputmethod.latin.R.string.auto_correction_threshold_mode_index_modest)
                : res.getString(io.makerforce.inputmethod.latin.R.string.auto_correction_threshold_mode_index_off);
        mBigramPredictionEnabled = readBigramPredictionEnabled(prefs, res);
        mDoubleSpacePeriodTimeout = res.getInteger(io.makerforce.inputmethod.latin.R.integer.config_double_space_period_timeout);
        ...
        mKeyPreviewShowUpDuration = Settings.readKeyPreviewAnimationDuration(
                ...
                res.getInteger(io.makerforce.inputmethod.latin.R.integer.config_key_preview_show_up_duration));
        mKeyPreviewDismissDuration = Settings.readKeyPreviewAnimationDuration(
                prefs, DebugSettings.PREF_KEY_PREVIEW_DISMISS_DURATION,
                res.getInteger(io.makerforce.inputmethod.latin.R.integer.config_key_preview_dismiss_duration));
        final float defaultKeyPreviewShowUpStartScale = ResourceUtils.getFloatFromFraction(
                res, io.makerforce.inputmethod.latin.R.fraction.config_key_preview_show_up_start_scale);
        final float defaultKeyPreviewDismissEndScale = ResourceUtils.getFloatFromFraction(
                res, io.makerforce.inputmethod.latin.R.fraction.config_key_preview_dismiss_end_scale);
    ...
    private static boolean readBigramPredictionEnabled(final SharedPreferences prefs,
        ...
        return prefs.getBoolean(Settings.PREF_BIGRAM_PREDICTIONS, res.getBoolean(
                io.makerforce.inputmethod.latin.R.bool.config_default_next_word_prediction));
    }

    private static float readAutoCorrectionThreshold(final Resources res,
            final String currentAutoCorrectionSetting) {
        final String[] autoCorrectionThresholdValues = res.getStringArray(
                io.makerforce.inputmethod.latin.R.array.auto_correction_threshold_values);
    ...
    private static boolean needsToShowVoiceInputKey(final SharedPreferences prefs,
        ...
        if (prefs.contains(Settings.PREF_VOICE_MODE_OBSOLETE)) {
            final String voiceModeMain = res.getString(io.makerforce.inputmethod.latin.R.string.voice_mode_main);
