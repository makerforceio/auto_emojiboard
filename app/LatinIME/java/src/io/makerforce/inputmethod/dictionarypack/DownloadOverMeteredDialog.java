...
public final class DownloadOverMeteredDialog extends Activity {
    ...
    protected void onCreate(final Bundle savedInstanceState) {
        ...
        setContentView(io.makerforce.inputmethod.latin.R.layout.download_over_metered);
        setTexts(localeString, size);
    }

    private void setTexts(@Nullable final String localeString, final long size) {
        final String promptFormat = getString(io.makerforce.inputmethod.latin.R.string.should_download_over_metered_prompt);
        final String allowButtonFormat = getString(io.makerforce.inputmethod.latin.R.string.download_over_metered);
        final String language = (null == localeString) ? ""
                : LocaleUtils.constructLocaleFromString(localeString).getDisplayLanguage();
        final TextView prompt = (TextView)findViewById(io.makerforce.inputmethod.latin.R.id.download_over_metered_prompt);
        prompt.setText(Html.fromHtml(String.format(promptFormat, language)));
        final Button allowButton = (Button)findViewById(io.makerforce.inputmethod.latin.R.id.allow_button);
